  ...
  return (
    <Screen refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} />}>
      ...
      {items.length === 0 ? (
        ...
        items.map((i) => (
          <Pressable key={i.id} onPress={() => router.push(`/(landlord)/inquiries/${i.id}`)}>
            <Card style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
              <View style={{ flex: 1 }}>
                <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
                  <Body style={{ fontWeight: "700" }}>{i.guestName}</Body>
                  {i.unread > 0 ? (
                    <View style={{ minWidth: 18, height: 18, borderRadius: 9, backgroundColor: colors.primary, alignItems: "center", justifyContent: "center", paddingHorizontal: 5 }}>
